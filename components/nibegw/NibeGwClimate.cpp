...
void NibeGwClimate::publish_current(float value) {
...
void NibeGwClimate::restart_timeout() {
...
void NibeGwClimate::setup() {
  if (this->sensor_) {
    this->sensor_->add_on_state_callback([this](float state) { this->publish_current(state); });
    this->publish_current(this->sensor_->state);
  }
  ...
  this->gw_->set_request(address_, RMU_WRITE_TOKEN, [this] {
    ...
    data_.erase(it);
    return write_token(RMU_WRITE_TOKEN, payload);
  });

  /* setup response to accessory information */
  this->gw_->set_request(address_, ACCESSORY_TOKEN,
                         write_token(ACCESSORY_TOKEN, set_u16_index(0xEE, RMU_DEVICE_VERSION)));

  /* setup response to something odd we don't know, copied from nibepi */
  this->gw_->set_request(address_, RMU_DATA_TOKEN, write_token(RMU_WRITE_TOKEN, {0x63, 0x00}));
  ...
  this->gw_->add_listener(address_, RMU_DATA_MSG, [this](const request_data_type &message) {
    ...
    if (message[RMU_DATA_OFFSET_FLAGS0] & (RMU_DATA_FLAGS0_USE_ROOM_SENSOR_S1 << this->index_)) {
      this->target_temperature = get_u8_decimal(&message[RMU_DATA_OFFSET_TARGET_TEMPERATURE_S1 + this->index_], 0.1, 50);
    } else {
      this->target_temperature = NAN;
    }

    /* this field has an added 0.5 degrees, to trigger rounding in RMU, subtract off before checking invalid values */
    this->current_temperature = get_s16_decimal(get_u16(&message[RMU_DATA_OFFSET_CURRENT_TEMPERATURE_SX])-5, 0.1, 0);
...
void NibeGwClimate::control(const climate::ClimateCall &call) {
  ...
  if (call.get_target_temperature().has_value()) {
    if (this->mode == climate::CLIMATE_MODE_AUTO) {
      ESP_LOGW(TAG, "Unable to set temperature in auto mode");
    } else {
      auto target = *call.get_target_temperature();
      this->publish_target(target);
    }
  }
}
